...
- *Task dispatch subagents:* Spawned by the Manager via `Agent(subagent_type="apm-worker", prompt=...)` to execute Tasks. These are the primary execution mechanism - ephemeral, one per dispatch unit. The `apm-worker` agent definition provides behavioral rules (execution procedure, iteration limits, logging formats) as the system prompt; the Task Prompt content is passed as the `prompt` parameter. They write Task Logs and return a structured result. They cannot spawn nested subagents (`disallowedTools: Agent` in the agent definition).
...
**Task Prompt construction** - The Manager assembles each Task Prompt as a self-contained document, sets up version control (feature branch for sequential, worktree for parallel), and dispatches via `Agent(subagent_type="apm-worker", description="...", prompt=<task_prompt_content>)`. The `apm-worker` agent definition provides behavioral rules as the system prompt; the `prompt` parameter contains only the task-specific content. For batch dispatch, the prompt contains a batch envelope with multiple Task Prompts. For parallel dispatch, multiple `Agent()` calls in a single message run concurrently - still foreground, the Manager blocks until all return. All dispatch runs in the foreground by default. Background dispatch is a separate concern (allowing the Manager to continue other work while subagents run) and is only used when the User explicitly requests it and confirms proper Claude Code permission configuration.

**Follow-up Task Prompts** - When a Task Review determines retry is needed, the Manager issues a follow-up via a fresh `Agent()` call with a refined prompt. Subagents are ephemeral - there is no mechanism to continue a completed subagent. Each follow-up gets a clean context informed by the previous attempt's Task Log. The follow-up uses the same log path as the original (the subagent overwrites the previous log).

### 7.2 Task Execution

**Runtime:** `agents/apm-worker.md`

The `apm-worker` agent definition provides the subagent's behavioral rules as its system prompt: execution procedure, iteration limits, logging formats, and version control standards. The subagent executes Task instructions, validates results, iterates if needed, logs the outcome, and returns a structured result.
