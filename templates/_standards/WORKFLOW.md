...
**Task Prompt construction** - The Manager assembles each Task Prompt as a self-contained document, sets up version control (feature branch for sequential, worktree for parallel), and dispatches via `Agent(subagent_type="apm-worker", description="...", prompt=<task_prompt_content>)`. The `apm-worker` agent definition provides behavioral rules as the system prompt; the `prompt` parameter contains only the task-specific content. For batch dispatch, the prompt contains a batch envelope with multiple Task Prompts. For parallel dispatch, multiple `Agent()` calls in a single message run concurrently. All dispatch runs in the foreground by default - the Manager blocks until the subagent returns. Background dispatch is only used when the User explicitly requests it and confirms proper Claude Code permission configuration.
